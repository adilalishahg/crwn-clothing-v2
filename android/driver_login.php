...
if (isset($_REQUEST['username']) && isset($_REQUEST['password'])) {
	...
	$jsonarray = array();
		  ...
		  $jsonarray['error'] = 'Login Failed';
			  ...
			  $querylogin = "UPDATE drivers SET login_status = '1',udid='".sql_replace($_REQUEST['udid'])."' WHERE Drvid = '".sql_replace($data[0]['Drvid'])."' ";
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'logout'){
		if (isset($_REQUEST['driverID'])) {
			$querylogout = "UPDATE drivers SET login_status = '0',udid='' WHERE   drv_code = '".sql_replace($_REQUEST['driverID'])."' ";
			//$querylogout = "UPDATE drivers SET login_status = '0',udid='' WHERE   Drvid = '".sql_replace($_REQUEST['driverID'])."' ";
