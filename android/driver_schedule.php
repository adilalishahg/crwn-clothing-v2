...
if($db->query($queryD) && $db->get_num_rows() > 0)	{	$dataD = $db->fetch_one_assoc();	 }
	...
	td.drp_add,td.drp_time,td.trip_miles,td.wc,td.type,td.status,td.acknowledge_status,td.add_current, td.modiv_id,td.ccode as legid 
		  FROM trips as t INNER JOIN trip_details as td ON t.trip_id=td.trip_id
		    WHERE t.trip_date BETWEEN '$pday' AND '$today' AND td.acknowledge_status IN('0','1') AND td.status IN('2','10','5','6','9','12','13') AND td.drv_id ='$driverID'  ORDER by td.pck_time ASC";
		...
		for($i=0;$i<sizeof($tripdata);$i++){
			...
			$Q="SELECT vt.vehtype,h.pick_signature,h.drop_signature,h.account_name	,ri.patient_weight,ri.oxygen,ri.dstretcher,ri.bar_stretcher,ri.stretcher,ri.dwchair,ri.wchair,ri.escort,ri.org_apptime 
