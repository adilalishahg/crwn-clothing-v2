...
function convertinto24($time, $rad)
{
	if ($time == '00:00' || $time == '00:00:00') {
		...
		return date("H:i", strtotime("" . $time . " " . $rad . ""));
	}
}
...
function check_drv()
{
	...
	if ($db->query($Query) && $db->get_num_rows() > 0) {
		...
		if ($totalRows > ($_SESSION['limit'] - 1)) {
			...
			$_SESSION['driver_quota_message'] = '';
			$_SESSION['driver_quota'] = '';
		}
	} else {
		$_SESSION['driver_quota_message'] = '';
		$_SESSION['driver_quota'] = '';
	...
	$Queryhosp1 = "SELECT ac.id,ac.account_name FROM " . accounts . " ac  WHERE TRIM(LOWER(REPLACE(account_name,' ',''))) IN('logisticcare','logisticare','modivcare')";
	if ($db->query($Queryhosp1) && $db->get_num_rows() > 0) {
		$xyz = $db->fetch_one_assoc();
		$logiticid = $xyz['id'];
	}
	$_SESSION['logiticid'] = $logiticid;
	/**
	 * Account Query to get notifications in a drop down on main menu
	 */
	$accountQuery = "SELECT req.id, req.clientname, acc.account_name FROM request_info req JOIN accounts acc ON req.account=acc.id WHERE notification=1";
	if ($db->query($accountQuery) && $db->get_num_rows() > 0) {
