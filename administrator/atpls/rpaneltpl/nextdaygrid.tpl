...
$.post("dvmap2.php", {did: did, tid: tid, tdate: tdate, ttime: ttime, acknowledge_status: acknowledge_status}, function(data)
					{
						...
						{ //alert(data);
							...
							if(data == '0')
								...
								if(did==''){alert("Un-Assigned");}else{	alert("Assigned");	}		//alert(msg[2]); // location.reload(); 
								}
							/*$(	"#msg").html(msg[0]);
							if(msg[1] == '0')
							{
...
//document.getElementById("disable"+idd).disabled = true;
			location.reload();
		}else{	alert('You can not reject this ride.');	//alert(data);
		}
...
<!---->                   
 ...
 &nbsp;&nbsp;&nbsp;</a>  {if $yes eq '1'}{if $date neq $date0}{/if}{/if}
                </ul>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          ...
          <strong>Driver:</strong> <select name="driver" id="driver" onChange="fsubmit('nextdaygrid.php?st={$st}&date={$date}',this.value);">
                    ...
                    {/section}  
                                  ...
                                  <strong>Patient:</strong> <select  onchange="fsubmit2('nextdaygrid.php?st={$st}&date={$date}',this.value);" name="user" id="user">
              ...
              <tr>
                <td height="44" align="center"  valign="top" style=""><table width="100%" border="0" class="main_table1" cellpadding="0" cellspacing="0" >                  
					...
					<tr valign="top" id="{$membdetail[q].tdid}" {if $membdetail[q].drv_id neq ''  && $membdetail[q].drv_id neq '000000' && $membdetail[q].duplicate eq '0'} class="assign" {/if}	{if $membdetail[q].drv_id neq '' && $membdetail[q].duplicate eq '1'} class="duplicateassign" {/if} >
                      ...
                      {/section}</b><br/>
                      {$membdetail[q].ccode} {$membdetail[q].modiv_id}
                      {if $membdetail[q].modiv_detail_id neq ''}<br/>{$membdetail[q].modiv_detail_id}{/if}</td>
...
{/section} {/if}
                   ...
                   {$membdetail[q].pck_add}{if $membdetail[q].pickup_instruction neq ''}<br/>[<strong>Instruction:</strong> {$membdetail[q].pickup_instruction}]{/if}{if $membdetail[q].p_phnum neq ''}<br/>[<strong>Phone #:</strong> {$membdetail[q].p_phnum}]{/if}</td>
                      ...
                      {if $membdetail[q].wc eq '1'} W/C{else}
                       <span id="pick_time_2{$membdetail[q].tdid}">{$membdetail[q].pck_time|date_format:"%H:%M"}</span>
                       {if $membdetail[q].modiv_flage eq '1'}
                      <a id="pick_time_button_{$membdetail[q].tdid}" href="#" onClick="change_time2('{$membdetail[q].tdid}')"><img src="../graphics/schedule.png" height="24" width="24" title="Change Pick Up Time" ></a>
                      ...
                      <input type="button" value=" Updade " class="btn" onClick="change_time('{$membdetail[q].tdid}');" title="Update Pick Up Time" />
                      </span>
                      {/if}
                      {/if}
                      </td>
                     ...
                     <td align="left" valign="top" class="grid_content" >{if $membdetail[q].org_apptime neq ''}  {$membdetail[q].org_apptime}{else}--:--{/if}</td>
                      ...
                      {if $membdetail[q].modiv_flage eq '1' && ($membdetail[q].status eq '9' || $membdetail[q].status eq '5')} 
                 <a href="#" onClick="create_receipt_approve('{$membdetail[q].tdid}','reject');" title="Reject"> <img src="../graphics/disable.jpg" /></a>
                 <br>{/if}                 
                      
                      &nbsp;&nbsp;<a href="#" onClick="window.open('edit2.php?id={$membdetail[q].reqid}&date={$date}')" title="Edit"> <img border="0" alt="Edit" src="../graphics/edit.png"></a>&nbsp;&nbsp;			  
