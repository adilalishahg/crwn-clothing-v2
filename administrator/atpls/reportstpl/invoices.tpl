...
function create_receipt_approve(id,status){ 
	
	if(status=='reject') {
		ok=confirm("Are you sure you want to reroute of this Trip?");
	} else {
		ok=confirm("Are you sure you want to create receipt of this Trip?");
	}
		if (ok)
		{ //alert(st);		
	 $.post("../approve_receipt_ajax.php", {id: ""+id,status: ""+status}, function(data){ // alert(data);
	 	if(data==1){  
			alert('Invoice sent to the ModiVCare');
			location.reload();
		}
	});		
	}else {
...
<td align="left" valign="middle">{$invoicesdata[q].clientname}{if $invoicesdata[q].modiv_id neq ''}<img style="margin-top: -11px;" border="0" width="60" height="30" title="ModivCare" alt="ModivCare" src="../graphics/modiv30.svg" />{/if}</td>
...
<td align="left" valign="middle"> 
{if $invoicesdata[q].leg eq '1'}
{$invoicesdata[q].apptime}	{else}
{$invoicesdata[q].returnpickup}  {/if}</td>
...
<td align="center" valign="middle"><a href="javascript:popWind2('medical_invoice.php?id={$invoicesdata[q].tid}')" ><img alt="View Invoice detail" border="0"  src="../graphics/view.gif"></a><br/><br/>
{if $invoicesdata[q].account eq $smarty.session.logiticid}<a href="javascript:popWind2('edit_request2.php?id={$invoicesdata[q].tid}')" ><img alt="Edit Information" border="0"  src="../graphics/edit.png"></a>{else}
<a href="javascript:popWind2('edit_request.php?id={$invoicesdata[q].tid}')" ><img alt="Edit Information" border="0"  src="../graphics/edit.png"></a>
{/if}
{if $invoicesdata[q].modiv_id neq ''}<br/>
{if $invoicesdata[q].receipt_send_modivcare eq '1'}Receipt Sent to ModiVCare {else}
<input type="button" value="Create Receipt" onclick="create_receipt_approve('{$invoicesdata[q].tdid}','receipt');" title="Create Receipt" class="btn" />
{/if}
{/if}
