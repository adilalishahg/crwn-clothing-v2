 ...
 $query2 = "SELECT * FROM contact_info WHERE c_id = '1' ";
	   ...
	   $query="SELECT ac.id,ac.freemiles as frmiles,ac.account_name,ac.address,ac.city,ac.state,ac.zip,tr.* FROM request_info as tr LEFT JOIN ".accounts." as ac on tr.account=ac.id WHERE 1 $whr_clz  LIMIT 1";
		...
		$tdata = $db->fetch_one_assoc(); //print_r($tdata);
		 $queryB = "SELECT bl.*,tr.three_address,tr.four_address,tr.five_address,tr.triptype,tr.vehtype,tr.pickaddr,tr.destination,tr.backto,tr.clientname,tr.appdate,tr.apptime,tr.returnpickup,tr.three_pickup,tr.four_pickup,tr.phnum,tr.id as tid,tr.po,tr.legaid,tr.legbid,tr.legcid,tr.legdid,tr.dob FROM billing_info as bl left join ".TBL_FORMS." as tr on bl.trip_id=tr.id WHERE 1  AND tr.reqstatus = 'approved'  AND bl.cancel = '0' AND bl.status IN(1,4,6,7) $whr_clz ORDER BY bl.id";
