<?php
	...
	{ 
	   ...
	   elseif($status == ''){ $whr .= " AND td.status IN ('1','4','6','3','8','7','9','10') ";  }
		...
		//if($logiticid!= '')
		{$whr .= " AND t.account = '".$_REQUEST['logiticid']."'";}
     ...
     $query = "SELECT td.*,t.trip_clinic,tr.invoice_gen, CONCAT(d.fname,' ',d.lname) as name,t.trip_user, d.drv_code,t.account 
			...
			WHERE $whr ORDER BY td.aptime ASC  ";
		if($db->query($query) && $db->get_num_rows() > 0)
		{	$totalRows = $db->get_num_rows();
			$data = $db->fetch_all_assoc(); $yes = '1';
			for($i = 0;$i<sizeof($data);$i++)
		   {	if($data[$i]['invoice_gen']==0){ invoice_generation($data[$i]['reqid'],$db); }
  ...
  $Queryhosp1 = "SELECT ac.id,ac.account_name FROM ".accounts." ac  WHERE TRIM(LOWER(REPLACE(account_name,' ','')))='".strtolower(trim('ModivCare'))."' ORDER BY ac.account_name ASC";
