...
// Delete Category Script
    if(verify($_GET['delId'],"index.php")){
   		$QueryDel = "DELETE FROM ".companycodes." WHERE id='".$_GET['delId']."'";
...
// Fetch all categories list
	$QueryCounty = "SELECT COUNT(*) AS rows FROM ".companycodes;
     if($db->query($QueryCounty) && $db->get_num_rows())
	  {
      $data = $db->fetch_all_assoc();
      $totalRows = $data[0]['rows'];
	  }
	$Query = "SELECT * FROM ".companycodes." ORDER BY `company` ASC LIMIT $offset, $limit";
