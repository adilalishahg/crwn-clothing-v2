...
function gen_data($data,$db){
	...
	$p_sex 	  				= $data[14];
	$center_name 	  		= $data[39];
	$enroll_date  			= substr($data[40], 0, -4).'-'.substr($data[40], 4, -2).'-'.substr($data[40], 6); //20090501
							//yyyy-mm-dd
...
{	 if(1){	 $type = $_FILES['file_csv']['type'];
	  ...
	  else
	  {
	      ...
	      echo '<script>location.href="index.php";</script>';
			...
			$smarty->display('patientstpls/add_patients_xls.tpl');
...
[14] => F 
		...
		[37] => CMTY 
		[38] => Jacksonville 
[39] => KINGSLEY VILLAGE MEDICAL CENT 000105256 
		[40] => 20090501 
		[41] => 
		[42] => 
		[43] => 71 ) 
