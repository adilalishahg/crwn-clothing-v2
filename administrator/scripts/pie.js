...
//Get a pie chart=============================
  ...
  function getSlice(ColumnName,ColumnData,size,start,end,fillcolor,extrusion)
  {
		  ...
		  var poly ='<v:polyline title='+angle+' style="'+style+';z-index:'+(zIndex+1)+'"  points="'+r+'px,'+r+'px,'+x1 +','+y1+','+xy+','+x2+','+y2+'" strokeweight="0.1pt" stroked="f" strokecolor="#666666" fillcolor="'+fillcolor+'" strokeweight="1pt">';
			   ...
			   { 
			     ...
			     poly +='</v:polyline>';
				 var info = ColumnName+ ':' + ColumnData +' - '+Math.ceil((ColumnData/SUM)*100)+'%';
				 poly +='<span title="'+info +'" onmouseover="this.style.zoom=1.2;this.style.zIndex=( Date.parse(new Date())/1000);"   onmouseout="this.style.zoom=0.8;" style="cursor:hand;font-size:13px;zoom:0.8;background:'+fillcolor+';filter:Alpha(opacity=80);border:solid 1px #cccccc; padding:1px;font-family:Verdana;z-index:'+(zIndex + 2 + DATA.length)+';position:absolute;left:'+(parseInt(legenx) - ColumnName.length*3 )+'px;top:'+(parseInt(legeny)-15)+'px;">'+info +'</span>';
			   }	  
		  return arc  +   poly ;
  }
  ...
  { 
	...
	VML = '<div onselectstart="event.returnValue=false; return false;" style=";filter:progid:DXImageTransform.Microsoft.Gradient(startColorStr=#FFFFFF, endColorStr=#DEDEDE, gradientType=0) progid:DXImageTransform.Microsoft.Shadow(color=#222222,direction=135,strength=2);background:white;;width:'+SIZE +'px;height:'+SIZE +'px;border:solid 1px #cccccc;text-align:left;"><div style="position:relative;margin:20px;">' + VML + '</div></div>';
	...
	//TargetObj.innerHTML =  VML ;
	window.VMLRedrawTimer = setTimeout("void(0)",100)
	window.onresize = function()
	{
	  clearTimeout(window.VMLRedrawTimer);
      window.VMLRedrawTimer = setTimeout(RedrawVML,100)
	  function RedrawVML()
	  {
	    //TargetObj.innerHTML =  VML;
	    //TargetObj.insertAdjacentHTML("AfterEnd",VML)
	  }
