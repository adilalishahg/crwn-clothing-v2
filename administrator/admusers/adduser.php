...
// Add User
 ...
 if(!$error){
     ...
     $chkUname = "SELECT * FROM ".TBL_ADMIN." WHERE admin_uname='$admuname'"; 
		...
		$Query2 = "SELECT admin_id FROM ".TBL_ADMIN." WHERE admin_level='1' ";
   if($db->query($Query2) && $db->get_num_rows() > 0)
	{
	    $admincount = $db->get_num_rows();
	   if($admincount>=$_SESSION['adminlimit']){$error .= "<img src='../images/arrow.jpg'>&nbsp;&nbsp;Your admin user quota limit reached at the end.<br>";}
	}	 
