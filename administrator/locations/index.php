...
// Delete Category Script
    if(verify($_GET['delId'],"index.php")){
   		$QueryDel = "DELETE FROM ".locations." WHERE id='".$_GET['delId']."'";
...
// Fetch all categories list
	$QueryCounty = "SELECT COUNT(*) AS rows FROM ".locations;
     if($db->query($QueryCounty) && $db->get_num_rows())
	  {
      $data = $db->fetch_all_assoc();
      $totalRows = $data[0]['rows'];
	  }
	$Query = "SELECT * FROM ".locations." ORDER BY `location` ASC LIMIT $offset, $limit";
   ...
   // Footer paging
	...
	$smarty->display('locations/index.tpl');
?>