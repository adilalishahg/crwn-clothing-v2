...
if ($fetch == 'onedriver' && $driver_id != '') {
	...
	$queryone = " SELECT CONCAT('Driver Name:', fname,' ',lname,' Phone#: ',day_phnum,'<br> Speed: ',speed) as drinfo,lat,clockstatus,longt,trip_assingment,trip_status, clockstatus,fname as firstname, lname as lastname FROM drivers WHERE drv_code='$driver_id' AND login_status = '1' AND clockstatus = 'in' LIMIT 1";
...
if ($fetch == 'allsch') {
	...
	$sQuery = " SELECT CONCAT('Driver Name:', fname,' ',lname,' Phone#: ',day_phnum) as drinfo,speed,lat,longt,trip_assingment,clockstatus,trip_status, fname as firstname, lname as lastname  FROM drivers WHERE  drvstatus='Active' AND login_status = '1'  AND clockstatus = 'in' ";
