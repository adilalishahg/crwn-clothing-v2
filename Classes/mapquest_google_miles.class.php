...
class mapquest_google_miles
{
	public function distance($add1, $add2)
	{
		...
		try {
			$url = 'https://voice.hybriditservices.com/googleapi.php';
			$clienttoken_post = array('strSource' => 'Greco', 'SourceType' => 'Distance Matrix', 'apikeyused' => 'AIzaSyBBE53xDKH93kCWSWREyehlzH8N_t2R2lw');
			$curl = curl_init($url);
	...
	{
		$dp = '';
		$key = "AIzaSyBBE53xDKH93kCWSWREyehlzH8N_t2R2lw"; // Google Key: this key can be use only for testing purpose please replace it on live 
		try {
			$url = 'https://voice.hybriditservices.com/googleapi.php';
			$clienttoken_post = array('strSource' => 'Greco', 'SourceType' => 'Distance Matrix', 'apikeyused' => 'AIzaSyBBE53xDKH93kCWSWREyehlzH8N_t2R2lw');
			$curl = curl_init($url);
...
return(round($miles,2));*/
	...
	public	function reverstime($time)
	{
		...
		if ($hour > 12 && ($hour - 12) < 10) {
			$returnpickup = '0' . ($hour - 12) . ':' . ($minutess);
			$mad = 'pm';
		}
		if ($hour > 12 && ($hour - 12) > 9) {
			$returnpickup = ($hour - 12) . ':' . ($minutess);
			$mad = 'pm';
		}
	...
	public function hoursconvert($time, $ampm)
	{
		...
		if ($ampm == pm) {
			if ($hour != 12) {
				$time = ($hour + 12) . ':' . ($minutess);
			}
		}
		return $time;
	} 	//	return in amp pm fromate i.e 24 hour formate.	
