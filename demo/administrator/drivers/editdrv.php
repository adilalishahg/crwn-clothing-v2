<?php
 ...
 if(isset($_POST['submit'])){
  ...
  if(!$error) {
	  //for check limited driver according to quota limit
	  	$sqlDrv = $db->query("select * from ".TBL_DRIVERS." WHERE drvstatus='Active'");
		$totalActDrv = $db->get_num_rows();
		if($totalActDrv<10 || $totalActDrv<=10 && ($drvstatus=='Suspended' || $drvstatus=='Left')){
			$Query="UPDATE ".TBL_DRIVERS." SET drvstatus='$drvstatus' WHERE Drvid='$vid'";
			$db->execute($Query);
		}
