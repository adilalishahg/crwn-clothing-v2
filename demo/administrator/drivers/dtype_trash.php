...
{*/
	...
	{
		$id = $_GET['resid'];
		$queryRestore = " Update ".TBL_DRVTYPES." SET
																del = '0',
																del_by = '',
																del_date = ''
											where  dtype_id = '$id'";
		...
		{  
			echo "<script>alert('Driver type restored successfully!');</script>";
			echo "<script>location.href= 'dtype_trash.php';</script>";
		}
...
//}
	...
	$level =  $_SESSION['admuser']['admin_level'];
	if($level =='0')
	{
		$whr = '';
	}
	else
	{
	 ...
	 $Querypg = "SELECT COUNT(*) FROM ".TBL_DRVTYPES." Where del = '1' $whr " ;	
	...
	$Query2 = "SELECT * FROM ".TBL_DRVTYPES." Where del = '1' $whr LIMIT ".$pagination->startRow . ",".$pagination->maxRows;
...
/*	  echo "<pre>";
	...
	$smarty->display('drvtpl/dtype_trash.tpl');
