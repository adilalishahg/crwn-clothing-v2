        ...
        <tr>
          <td height="44" colspan="2" align="center"  valign="top"><form name="searchReport" action="" method="post" id="adduser">
            ...
            {section name=q loop = $drivers}
                      <option value="{$drivers[q].drv_code}" {if $drivers[q].drv_code eq $post.driver_id} selected{/if}>{$drivers[q].fname} {$drivers[q].lname}  - [ {$drivers[q].drv_code} ]</option>
            {/section}
                    </select></span>
        ...
        <tr>
          <td height="44" colspan="2" align="center"  valign="top" style="padding-bottom:50px;"><table width="100%" border="0" class="main_table11">
              ...
              <tr id="tr{$reqdetails[q].id}" bgcolor="{cycle values="#ffffff,#dfedfa"}">
                ...
                <td align="center" valign="top">{$data[q].uploadedon|date_format:"%D"} </td>
                <td align="center" valign="top">{$data[q].driver}</td>
                <td align="center" valign="top">{$data[q].vehicle}</td>
                
                <td align="center" valign="top">{$data[q].current_vehicle_milage}</td>
                <td align="center" valign="top">{$data[q].total_galon}</td>
                <td align="center" valign="top">{$data[q].price_per_galon}</td>
                <td align="center" valign="top">{$data[q].receipt_amount|string_format:"%.2f"}</td>
